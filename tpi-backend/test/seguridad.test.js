...
const usuarioAdmin = { usuario: "Administrador", clave: "123" };
const usuarioMiembro = { usuario: "Juan", clave: "123" };


describe("POST /api/login Administrador", function () {
  it("Devolveria error de autenticacion, porque tiene clave erronea", async function () {
    const res = await request(app)
      .post("/api/login")
      //.set("Content-type", "application/json")
      .send({ usuario: "Administrador", clave: "erronea" });

    expect(res.statusCode).toEqual(200);
    expect(res.body.message).toEqual("usuario or clave incorrecto");
  });

  it("Devolveria el token para usuario Administrador", async function () {
...
describe("GET /api/jwt/prendas", () => {

  it("Devolveria error, porque falta token de autorizacion", async function () {
    const res = await request(app).get("/api/jwt/prendas");
  ...
  it("Devolveria todos las prendas, solo autorizado para administradores", async function () {
